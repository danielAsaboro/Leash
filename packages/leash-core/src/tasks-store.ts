/**
 * File-based task store — `data/leash-tasks.json`. Moved into `@mycelium/leash-core` (web
 * keeps a re-export shim) so the `leash-tools-mcp` Tasks group and the web process share ONE
 * implementation over the same file.
 *
 * Writers: the dashboard UI (CRUD at /tasks), the assistant (create_task/update_task chat
 * tools, stamped source:"assistant"), the dreaming pass (source:"dream"). First load migrates
 * legacy `data/leash-dreams.json` consolidations into tasks. Mutations go through an
 * in-process mutex NESTING a cross-process file lock + a fresh read per edit + atomic rename,
 * so concurrent web/daemon writes never lose updates.
 */
import { generateId } from "ai";
import { join } from "node:path";
import { readJson, readJsonCached, writeJson, invalidateJsonCache } from "./json-store.ts";
...
/** Legacy dreaming consolidation row (migrated on first load). */
...
function mutate<T>(fn: () => Promise<T>): Promise<T> {
  return withLock(() => withFileLock(TASKS_FILE, fn));
}
