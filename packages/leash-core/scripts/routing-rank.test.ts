...
const localChat: RouteOption = { tier: "device", alias: "chat", tags: { modality: "text", paramClass: "small", specialist: "general" }, pricePerKiloToken: 0, inflight: 0 };
const peerBig: RouteOption = { tier: "private", alias: "chat-xl", peerKey: "PK_PRO", meshId: "primary", modelSrc: "src://big", tags: { modality: "text", paramClass: "large", specialist: "general" }, pricePerKiloToken: 0, inflight: 0 };
const publicBig: RouteOption = { tier: "public", alias: "chat-xl", peerKey: "PK_PUB", meshId: "open", tags: { modality: "text", paramClass: "large", specialist: "general" }, pricePerKiloToken: 10, inflight: 0 };
...
function main() {
  ...
  let r = rankRoutes({ bar: easyBar, sensitivity: "private", options: [localChat, peerBig] });
  assert.equal(r[0]?.alias, "chat", "idle local should win an easy turn");

  // 2. Hard bar → local 'small' is filtered out; the 'large' peer wins.
  r = rankRoutes({ bar: hardBar, sensitivity: "private", options: [localChat, peerBig] });
  ...
  r = rankRoutes({ bar: hardBar, sensitivity: "shareable", options: [localChat, publicBig] });
  ...
  const busyLocal = { ...localChat, inflight: 3 };
  const freePeerSmall: RouteOption = { tier: "private", alias: "chat", peerKey: "PK_FREE", meshId: "primary", modelSrc: "src://s", tags: { modality: "text", paramClass: "small", specialist: "general" }, pricePerKiloToken: 0, inflight: 0 };
  ...
  r = rankRoutes({ bar: { modality: "vision", minParamClass: "small" }, sensitivity: "private", options: [localChat] });
