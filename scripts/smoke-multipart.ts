...
async function main(): Promise<void> {
  ...
  assert.equal(model?.data.toString("utf8"), "stt", "model field");
  ...
  console.log(`✅ multipart — boundary parsed, model="stt", file "clip.wav" (${file!.data.length} bytes) round-trips byte-identical — GO`);
}
