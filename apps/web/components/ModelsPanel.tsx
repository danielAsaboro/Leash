...
export function ModelsPanel({ inventory, serve, catalog: initialCatalog, downloads: initialDownloads }: { inventory: ModelsInventory; serve: ServeStatus; catalog: CatalogModel[]; downloads: DownloadStatus[] }) {
  const router = useRouter();
  // The server render passes whatever catalog dump exists on disk — EMPTY on a fresh install (the
  // SDK catalog is generated by a spawned child, not at page render). Fetch it on mount so the
  // "Available" list populates: GET /api/leash/models?catalog=1 regenerates the dump if missing.
  const [catalog, setCatalog] = useState<CatalogModel[]>(initialCatalog);
  useEffect(() => {
    if (initialCatalog.length > 0) return; // already have it (server render found a dump)
    let alive = true;
    fetchWithTimeout("/api/leash/models?catalog=1", { cache: "no-store" }, TIMEOUT.heavy)
      .then((r) => r.json())
      .then((d: { catalog?: CatalogModel[] }) => {
        if (alive && Array.isArray(d.catalog)) setCatalog(d.catalog);
      })
      .catch(() => {
        /* leave the (empty) list; the user can retry via the refresh control */
      });
    return () => {
      alive = false;
    };
  ...
  const row = (r: TaggedRow) => (
    <tr key={`${r.category}:${r.alias ?? ""}:${r.name}:${r.cacheFile ?? ""}`}>
      ...
      <Cell mono>
        {/* Show the ctx slider for ANY configured model (CtxSizeControl defaults to 4096 when unset),
            so a freshly-added model can have its context window set — not just ones that already had it. */}
        {r.inConfig && r.alias ? (
          <CtxSizeControl
            row={r}
            busy={busy}
