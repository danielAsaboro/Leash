// apps/web/lib/leash/leash-defaults.ts
/**
 * Leash prompt defaults — no server-only guard so they can be imported by:
 *   • apps/web/lib/leash/tools.ts (re-exports them to server-only callers)
 *   • apps/web/lib/leash/main-agent.ts (reads the default agent markdown)
 *   • apps/web/scripts/*.test.ts (tsx assertion scripts)
 *
 * Keep this file side-effect-free and dependency-free.
 */
