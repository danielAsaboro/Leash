 ...
 * (Home Assistant token/URL, SearXNG URL, future keys). Replaces scattering secrets
 * across env vars.
 *
 ...
 *
 * Threat model (mirrors Odysseus `secret_storage.py`): protects a stolen DB/backup or
 * a leaked container layer — the ciphertext is useless without the key file. Does NOT
 * protect against process compromise (anyone who can read this module's memory or the
 * key file gets plaintext). For a single-user on-device exocortex that's the right bar.
 *
 * NOT marked `server-only`: imported by Next routes AND the research tsx child (which
 * reads the SearXNG URL); it holds no secrets in code, only the access path.
 */
