...
export interface MeshStatus {
  ...
  /** Whether this device can write to the mesh (null = mesh offline / daemon down). */
  writable: boolean | null;
  /** Short mesh id (autobase key prefix) — same on every member of one mesh. */
  meshId: string | null;
...
export async function meshStatus(): Promise<MeshStatus> {
  ...
  try {
    ...
    if (!r.ok) error = `Hypha shim answered ${r.status}`;
      ...
      const body = (await r.json()) as { peers?: MeshPeer[]; writable?: boolean | null; meshId?: string | null; forgotten?: string[] };
      peers = body.peers ?? [];
  ...
  return { peers, borrow, writable, meshId, forgotten, error };
}
