...
**`understory_today`** — the current edition. Call `context_run` with action `understory_today` for "what's in my paper", "what's new", "catch me up". Summarize the highlights conversationally rather than dumping raw text.

**`understory_search`** — find past coverage. Call `context_run` with action `understory_search` when they ask whether the paper covered a topic or want items about something specific. Pull the relevant pieces and quote/link them.
