...
**List** with `tasks_run` action `list_tasks` before answering "what's on my list / what's next / what's still open" — and before creating, so you don't duplicate an existing todo. Filter by status when they ask specifically (open vs in-progress vs done).

**Create and update** through `tasks_run`: use action `create_task` when the user expresses a new commitment, and action `update_task` as work moves. Mark done when they say it's finished, in-progress when they start, and bump priority when they flag something urgent. When they say "I finished the X", find the matching todo and close it rather than asking them to specify an id.
