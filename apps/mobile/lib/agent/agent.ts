 ...
 * single-use `ToolLoopAgent` that wraps a model + tool registry + per-turn instructions and runs
...
export type LeashTurn = {
  ...
  model: LanguageModel;
  /** Fully composed instructions (identity + soul/goals + memories + skill body + notes). */
  instructions: string;
...
export function buildLeashAgent(turn: LeashTurn): ToolLoopAgent<never, ToolSet> {
  ...
  return new ToolLoopAgent<never, ToolSet>({
    ...
    stopWhen: isStepCount(hasTools ? (turn.maxSteps ?? 8) : 1),
  });
}
