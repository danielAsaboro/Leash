...
of `@qvac/cli` (`mycelium/patches/@qvac+cli+0.6.0.patch`) that:
...
This extends QVAC's own serve — it does not route around it. The change is written up for upstream
in `submission/qvac-vision-pr.md`.
...
After the one-time warm-cache, the system runs in airplane mode. The enumerated non-AI calls are
recorded in `submission/day7-audit.md`; the structured remote-API-call disclosure file is a
tracked submission artifact **(hold until filed)**.
