...
are embedded with the already-loaded `embed` model, and the turn is matched by similarity.
...
```mermaid
flowchart TB
    Prompt["User turn"] --> Match["Match skills · RRF over embed<br/>(embedding + lexical floors)"]
