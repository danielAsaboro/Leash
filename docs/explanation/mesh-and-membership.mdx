...
- **Visibility** — `private` (your own encrypted circle) or `public` (an untrusted discovery cell).
- **Reach** — `local` (trusted compute-bearing peers) versus public providers selected only for an
  explicit shareable job over their separate authenticated transport.
